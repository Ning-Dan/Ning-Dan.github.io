...
export const lessonContent: Record<string, LessonDetail> = {
  "control-to-vla": {
    ...
    theory: [
      "经典状态反馈写成 uₜ=κ(xₜ)。VLA 则由图像、语言和本体状态条件化，一次生成未来 H 步动作。它可以像 MPC 一样滚动执行：预测一段、只执行前 E 步、重新观测；但它没有显式动力学与代价函数，而是从示范分布中学习。",
      "工程上让 VLA 输出末端增量、关节位置参考、gripper 命令或技能子目标。反归一化、坐标系变换、IK、碰撞、限速与高频跟踪属于下游确定性系统。VLA 常为 5–50 Hz，伺服环常为 100–1000 Hz。",
    ],
    formula: {
  ...
  "multimodal-transformer": {
    ...
    theory: ["图像经视觉编码器成为 patch，语言经 tokenizer，本体状态经投影，动作既可为离散词表 token，也可为连续 action slots。它们投影到共同隐藏维后交互。", "Mask 决定 causal、prefix-LM 或 action suffix 内双向通信。OpenVLA 是自回归离散动作；π₀ 连续 action suffix 使用双向注意力，不能画成同一结构。"],
  ...
  "action-representations": {
    ...
    theory: ["离散 bin 训练简单但有量化误差；FAST 对 action chunk 做 DCT、量化和 BPE，压缩高频序列；diffusion/flow 直接生成连续动作。", "跨 embodiment 时 padding 到同维数不等于语义统一。每个机器人必须定义 command type、frame、rotation、unit、dt、gripper、valid mask、统计量与逆变换。"],
    ...
    pitfalls: ["mm/m 混用", "spatial/body delta 混用", "velocity↔delta 忘记 dt", "padding 与真实零不可分"],
  ...
  "action-chunking": {
    ...
    formula: { latex: String.raw`1\le E\le H,\quad T_{\rm react}\approx E\Delta t_c,\quad T_{\rm chunk}=H\Delta t_c`, symbols: [
      { symbol: "E", meaning: "实际执行动作数。" }, { symbol: "H", meaning: "预测动作数。" }, { symbol: "Treact", meaning: "对新观测反应的近似间隔。" }, { symbol: "Tchunk", meaning: "chunk 覆盖的物理时间。" }, { symbol: "Δt_c", meaning: "动作采样周期。" },
    ] },
  ...
  "diffusion-policy": {
    lead: "Diffusion Policy 把动作块当条件生成对象：从噪声逐步去噪，表达同一观测下多个合理动作模式。",
  ...
  "flow-matching": {
    ...
    theory: ["本站统一 τ=0 噪声、τ=1 动作。线性路径 Xτ=(1−τ)ε+τA 的速度是 A−ε；推理从 ε 用正步长到 A。", "openpi 用相反约定：t=1 噪声、t=0 动作，目标 ε−A 并用负 dt。二者等价且自洽；π₀ 论文正文符号容易误读，教程不逐字照抄。"],
  ...
  pi05: {
    ...
    theory: ["π₀ 用 VLM prefix + flow action expert 生成连续动作。π₀.₅ 先输出文字子任务，如‘拿起枕头’，再据此生成 50 步、约 1 秒低层动作；离散语义路径和连续动作路径共享模型。", "它通过机器人动作与语义任务 co-training，尽量保留互联网语义知识。开放世界实验强调新环境泛化，不等于任意新机器人零样本可用。当前 openpi 公开实现只支持 π₀.₅ 的 flow-matching head，不能声称完整复现论文的混合高层路径。"],
    ...
    ], note: "这是层级条件分解。π₀.₅ 仍不是世界模型：它没有必须预测执行候选动作后的未来状态。" },
    practice: { title: "openpi π₀.₅ / LIBERO", summary: "数据转换→norm stats→pi05_libero→policy server→仿真 client。", steps: ["独立 .venv", "random-observation inference", "核对 norm/action shape", "云端微调", "单任务 rollout"], acceptance: ["H×dₐ finite", "训练推理同统计", "超时不重放旧 chunk", "记录版本/seed/GPU"], status: "云端必做", code: "uv run scripts/compute_norm_stats.py --config-name pi05_libero\nuv run scripts/train.py pi05_libero --exp-name=course_run --overwrite" },
    pitfalls: ["宣称新机器人零样本万能", "承诺 8GB 微调", "混装 JAX/Isaac 环境", "只看高层文字不看动作成功"],
  ...
  "data-and-adaptation": {
    ...
    theory: ["数据必须含 episode、时间戳、多相机、本体、语言、动作与动作契约。按 episode/场景切分；统计只用训练集。先 replay 和一批过拟合，再大训练。", "LoRA 只减少可训练权重，不消除激活显存，也不自动适配新 action dimension。OFT 以连续 action、chunk、并行解码和 L1 提升 OpenVLA 的速度与性能，但仍需目标域数据和 rollout。"],
  ...
  "vla-families": {
    ...
    theory: ["OpenVLA 是离散自回归动作 token；OFT 改为并行连续 chunk。SmolVLA 与 LeRobot 紧密，适合首个可跑 VLA。Octo 强调开放通用策略。π₀.₅ 用 flow+语义 co-training。GR00T N1.7 面向人形与部署全栈，但算力/厂商栈重。", "同任务先做 ACT/Diffusion baseline。窄任务上小策略可能更快更稳；VLA 优势主要是语义与迁移，不应预设一定赢。"],
    ...
    review: ["为何 SmolVLA 适合第一款？", "OFT 改了什么？", "ACT 何时更合适？"], completion: "用约束和证据选模型。",
    sources: [{ title: "SmolVLA", url: "https://huggingface.co/docs/lerobot/smolvla", role: "轻量" }, { title: "OFT", url: "https://openvla-oft.github.io/", role: "连续动作" }, { title: "GR00T", url: "https://github.com/NVIDIA/Isaac-GR00T", role: "人形" }, { title: "Octo", url: "https://octo-models.github.io/", role: "通用策略" }],
  },
  ...
  "world-models": {
    ...
    formula: { latex: String.raw`\hat{\mathbf A}=\arg\min_{\mathbf A}\;\mathbb E_{p_\phi(\mathbf z_{t+1:t+H}\mid\mathbf z_t,\mathbf A)}\left[\sum_{h=1}^{H}c(\mathbf z_{t+h},\ell)\right]`, symbols: [
      { symbol: "Â", meaning: "世界模型评估后选出的动作块。" }, { symbol: "A", meaning: "VLA/优化器候选。" }, { symbol: "pφ", meaning: "未来潜状态分布。" }, { symbol: "zₜ", meaning: "当前潜状态。" }, { symbol: "H", meaning: "imagined horizon。" }, { symbol: "c(z,ℓ)", meaning: "语言目标代价。" }, { symbol: "E", meaning: "对随机未来取期望。" },
    ] },
  ...
  capstone: {
    ...
    ], note: "不要只报告 5/5；样本太小时不确定性很大。" },
    practice: { title: "最终交付包", summary: "数据卡、模型卡、配置、测试、视频、失败分类和部署接口。", steps: ["安全边界", "数据审计", "ACT", "VLA", "三项消融", "扰动评测", "真机灰度"], acceptance: ["一键 smoke/eval", "checkpoint 可恢复", "固定 seed", "无越界碰撞", "失败可追溯"], status: "云端必做" },
